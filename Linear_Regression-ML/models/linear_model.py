...
class LinearModel(object):
    ...
    def __init__(self, ndims, w_init='zeros', w_decay_factor=0.001):
        ...
        if w_init == "zeros":
            self.w = np.array([[0.]]*(ndims+1))
        elif w_init == "ones":
            self.w = np.array([[1.]]*(ndims+1))
        else:
            self.w = np.random.uniform(size=(ndims+1,1))
        self.x = None
    ...
    def forward(self, x):
        ...
        """
        self.x = np.append(x, [[1]]*x.shape[0], axis=1)     # self.x is now (N, ndims+1)
        f = np.dot(self.w.T, self.x.T)    # x actually has to be (ndims+1, N) for w^T.x to work
        return f.T      # w^T.x returns (1, N), so must transpose again
